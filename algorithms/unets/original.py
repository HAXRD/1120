...
class EmulatorAttentionUNet(nn.Module):
    ...
    def forward(self, P_GUs, P_ABSs):
        ...
        # process input
        P_GUs1 = P_GUs.gt(0.).float()   # (32, 1, 64, 64)
        P_ABSs1 = P_ABSs.gt(0.).float() # (32, 1, 64, 64)

        x = torch.cat([P_GUs1, P_ABSs1], dim=1) # (32, 2, 64, 64)

        # encoding
        x1 = self.conv1(x)            # (32, 2, 64, 64) --> (32, 64, 64, 64)

        x2 = self.maxpool(x1)         # (32, 64, 64, 64) --> (32, 64, 32, 32)
        x2 = self.conv2(x2)           # (32, 64, 32, 32) --> (32, 128, 32, 32)

        x3 = self.maxpool(x2)         # (32, 128, 32, 32) --> (32, 128, 16, 16)
        x3 = self.conv3(x3)           # (32, 128, 16, 16) --> (32, 256, 16, 16)

        x4 = self.maxpool(x3)         # (32, 256, 16, 16) --> (32, 256, 8, 8)
        x4 = self.conv4(x4)           # (32, 256, 8, 8) --> (32, 512, 8, 8)
        ...
        # d4 = self.up4(d5)
        d4 = self.up4(x4)           # (32, 512, 8, 8) --> (32, 256, 16, 16)
        x3 = self.att4(g=d4, x=x3)  # (32, 256, 16, 16), (32, 256, 16, 16) --> (32, 256, 16, 16)
        d4 = torch.cat((x3, d4), dim=1) # (32, 256, 16, 16), (32, 256, 16, 16) --> (32, 512, 16, 16)
        d4 = self.up_conv4(d4)      # (32, 512, 16, 16) --> (32, 256, 16, 16)
